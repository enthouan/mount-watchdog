...
The installed file is root-owned and restrictive. Runtime validation rejects unsafe POSIX modes, ownership, parent paths, hard links where relevant, or symlinks rather than continuing with an untrusted privileged configuration. ACL validation is deliberately stricter than a write-rights allowlist: every `allow` entry and every noncanonical ACL is rejected. Canonical deny-only ACLs are accepted on repository source ancestors, privileged parent chains, and protected system inputs such as the autofs maps because they can only reduce access and include the normal macOS home-directory deny-delete shape. MountWatchdog-owned installed files, backup trees, lifecycle/runtime locks, logs, and state nodes must contain no ACL entries. Newly created managed nodes have inherited ACLs removed and are then verified; an existing managed node with an ACL fails closed and is never silently normalized.

The established volatile state root remains `/var/run/com.antoinemenard.mount-watchdog`. Before reading or creating state, the installed runtime verifies macOS's exact root-owned `/var -> private/var` alias, its non-writable `/private` ancestors, and the physical `/private/var/run` parent. The standard root-owned `daemon` group-write bit on `/private/var/run` is a narrow platform exception; world write is rejected, and the MountWatchdog state directory itself must be a real root-owned `0700` directory with trusted leaves.
...
Two stock macOS records receive narrow treatment. A single `+auto_master` Directory Service include may coexist with the required explicit file hook, but it is not accepted as an alternative route to `auto_smb`; Open Directory expansion remains unsupported until the [future migration](open-directory-migration.md) has owner-approved native validation. A single `/- -static` entry is accepted when `/etc/fstab` is absent or is a trusted regular file whose records fit the narrow inspected subset. Records must contain four through six whitespace-separated fields, exactly one `rw`, `ro`, `sw`, or `xx` mount-type atom, and numeric frequency/pass fields when present. `net` records feed the separate `-fstab` map; `sw` and `xx` records do not describe active direct filesystem targets; and the literal macOS local-volume target `none` cannot overlap an absolute selected path. Every other active `rw` or `ro` target must be a literal canonical absolute path and must be structurally disjoint from every selected MountWatchdog path. Unsupported syntax, symlinks including dangling symlinks, and equal/ancestor/descendant targets are rejected without printing the source field. Any other active direct map or include is rejected. These exceptions are residual inspection limits, not proof that no external mapping exists.

The runtime revalidates the supported hookup at tick start, immediately before each normal unmount, and immediately before a coalesced refresh. It requires trusted, single-link `/etc/auto_master` and `/etc/auto_smb` files, the exact explicit `/- auto_smb` or `/- /etc/auto_smb` entry, strict parsing of both maps, one exact map tuple for every installed selection, and—when `/- -static` is active—a trusted, single-link `/etc/fstab` whose active direct targets remain disjoint. Failure is configuration drift, not an outage: `autofs-hook-missing`, `autofs-master-map-invalid`, `autofs-selected-map-missing`, `autofs-selected-map-invalid`, `autofs-selected-mapping-mismatch`, `autofs-static-map-invalid`, or `autofs-static-map-conflict` is persisted without running a mount snapshot, TCP probe, unmount, or refresh. A later tick may re-evaluate the files, but it never repeatedly invokes `automount -c` while drift remains.
