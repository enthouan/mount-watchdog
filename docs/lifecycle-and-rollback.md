...
Starting with `v0.2.0`, the installer also accepts `--all` instead of names; see [selection options](setup.md#select-all-mappings). Every current map entry is validated and selected. Review additions on each invocation; removals still require `--replace-targets`.
